...
def _add_anonymous_reaction_event(grist_client, chat_id, message_id, reaction_update,
                                  chat_title='', chat_type='', user_id='', username='',
                                  first_name=''):
...
async def setup_reaction_handlers(grist_client):
    ...
    async def handle_message_reaction(reaction_update: MessageReactionUpdated):
        ...
        if reaction_update.user:
            ...
            if added_reactions > 0:
                ...
                if membership:
                    try:
                        grist_client.update_reaction_count(user_id, chat_id, added_reactions)
                        logger.info(f"✅ Updated reaction count for user {user_id}")
                        # Record one events-table row per tracked emoji added
                        for tracked_emoji in tracked_emojis:
                            grist_client.add_event({**event_base,
                                'user_id': str(user_id),
                                'username': username or '',
                                'first_name': first_name or '',
                    ...
                    # Create a new membership from this reaction
                    logger.info(f"📝 Creating new membership from reaction data for {username}")
                    new_user = {
                        'user_id': str(user_id),
                        'username': username or '',
                        'first_name': first_name or '',
                        ...
                        'last_reacted': datetime.now(),
                        'is_admin': False  # Default to not admin
                    }

                    try:
                        if grist_client.add_subscriber(new_user, chat_id=chat_id,
                                                       chat_title=chat_title, chat_type=chat_type):
                            logger.info(f"✅ Created new membership from reaction: {user_id}")
                            # Record one events-table row per tracked emoji added
                            for tracked_emoji in tracked_emojis:
                                grist_client.add_event({**event_base,
                                    'user_id': str(user_id),
                        ...
                        logger.error(f"❌ Error creating membership from reaction: {e}")
        else:
            ...
            try:
                ...
                if message and message.from_user:
                    ...
                    membership = grist_client.get_membership(msg_user_id, chat_id)
                    if membership and membership.get('is_admin'):
                        try:
                            grist_client.update_reaction_count(msg_user_id, chat_id, 1)
                            logger.info(f"✅ Updated reaction count for message author (admin) {msg_user_id}")
                            # Anonymous reaction: reactor identity unknown, user fields left empty
                            _add_anonymous_reaction_event(grist_client, chat_id, message_id,
                                                          reaction_update, chat_title, chat_type)
                        except Exception as e:
            ...
            if actor and hasattr(actor, 'user_id'):
                ...
                # Try to update membership by user_id
                membership = grist_client.get_membership(user_id, chat_id)
                if membership:
                    try:
                        grist_client.update_reaction_count(user_id, chat_id, 1)
                        logger.info(f"✅ Updated reaction count for user {user_id}")
            ...
            try:
                ...
                for admin in admins:
                    ...
                    if status == 'creator':
                        ...
                        if membership:
                            try:
                                grist_client.update_reaction_count(user_id, chat_id, 1)
                                logger.info(f"✅ Updated reaction count for channel creator {user_id}")
                                # Anonymous reaction: reactor identity unknown, user fields left empty
                                _add_anonymous_reaction_event(grist_client, chat_id, message_id,
                                                              reaction_update, chat_title, chat_type)
                            except Exception as e:
                ...
                if not creator_found:
                    ...
                    membership = grist_client.get_membership(f"admin_{bot_id}", chat_id)
                    if membership:
                        try:
                            grist_client.update_reaction_count(f"admin_{bot_id}", chat_id, 1)
                            logger.info(f"✅ Updated reaction count for bot admin {bot_id}")
                            # Anonymous reaction: reactor identity unknown, user fields left empty
                            _add_anonymous_reaction_event(grist_client, chat_id, message_id,
                                                          reaction_update, chat_title, chat_type)
                        except Exception as e:
                        ...
                        try:
                            grist_client.add_subscriber(admin_data, chat_id=chat_id,
                                                        chat_title=chat_title, chat_type=chat_type)
    ...
    async def handle_reaction_count_updated(count_update: MessageReactionCountUpdated):
        ...
        if chat_type == 'channel':
            ...
            channel_record = grist_client.get_membership(channel_user_id, chat_id)
            ...
            if channel_record:
                ...
                if total_reactions > 0:
                    try:
                        grist_client.update_reaction_count(channel_user_id, chat_id, total_reactions)
                        logger.info(f"✅ Updated reaction count for channel {chat_id}")
                    except Exception as e:
                ...
                try:
                    grist_client.add_subscriber(new_channel, chat_id=chat_id,
                                                chat_title=chat_title, chat_type=chat_type)
